...
- **Resume access** — `MEDIA_ROOT` is *not* served by any general route. Whitenoise only covers `STATIC_ROOT`, and the old `static(settings.MEDIA_URL, ...)` line in `salesfloor/urls.py` was `DEBUG`-only, so on Render every resume link 404'd. Resumes are now streamed by `download_resume` (`/resumes/<pk>/`, `@staff_member_required`) and the admin's `resume_link` column points there. Don't "fix" a future media 404 by adding a blanket `static()`/whitenoise media route — resume paths are guessable and these are candidate PII, so that would publish every resume to anyone with the URL.
